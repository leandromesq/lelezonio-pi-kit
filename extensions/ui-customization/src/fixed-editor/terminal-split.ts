...
export interface InternalTui extends PatchedRenderable {
  addInputListener?(
    listener: (
      data: string,
...
const CONTEXT_MENU_SELECTION_RESTORE_WINDOW_MS = 500;
...
export class TerminalSplitCompositor {
  private readonly tui: InternalTui;
  ...
  private restoreTerminalState(options: DisposeOptions = {}): void {
    const activeMode =
      this.extendedKeyboardMode ?? this.activeExtendedKeyboardMode();
    const shouldRestoreKeyboardMode =
      !options.resetExtendedKeyboardModes && activeMode !== null;
    ...
    this.originalWrite(
      beginSynchronizedOutput() +
        ...
        enableAlternateScrollMode() +
        exitAlternateScreen() +
        (shouldRestoreKeyboardMode && activeMode

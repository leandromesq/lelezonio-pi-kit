...
- Profile routing (`subagents.json`): all named profiles run on the Pi harness. `planner` uses `openai-codex/gpt-5.6-luna`, `coder` uses `opencode-go/deepseek-v4.1-flash`, and `reviewer` uses `opencode-go/deepseek-v4.1-flash`; profile-less spawns also default to the Pi harness. Override the harness to Codex only when Pi lacks a tool required by the task, such as an MCP integration available through Codex CLI.
